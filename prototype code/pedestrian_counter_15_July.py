    ...
    def bi_counting(self):
        ...
        while True:
            ...
            if self.count_2 >= detection_threshold and not self.pedestrian_detected_2:
                ...
                sensor_2_get_time = time.time()
                pseudo_count += 1

            if pseudo_count >= 2:
                if sensor_1_get_time < sensor_2_get_time:
                    self.no_of_pedestrians_1 += 1
                    print("In Count: " + str(self.no_of_pedestrians_1))
                else:
                    self.no_of_pedestrians_2 += 2
                    print("Out Count: " + str(self.no_of_pedestrians_2))
                pseudo_count = 0
                sensor_1_get_time = sensor_2_get_time = time.time()  # reset sensor 1 and sensor 2 time
